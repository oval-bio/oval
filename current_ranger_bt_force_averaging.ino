...
#define VBATREADLOOPS          10  //read vbat every this many OLED_REFRESH_INTERVAL loops, originally 100 for 200 ms oled refresh
...
#define ADC_CALIBRATE_FORCED       //uncomment to set manual offset/gain values below rc3105
#define ADC_CALIBRATE_FORCED_OFFSET 25
#define ADC_CALIBRATE_FORCED_GAIN   2061
//***********************************************************************************************************
...
#define OLED_REFRESH_INTERVAL 1000 //ms originally 200
...
//***********************************************************************************************************
#define SERIALBAUD 9600      //Serial baud for HC-06 bluetooth output
#define BT_OUTPUT_NANOS        //ADC | AMPS | NANOS Change format of bluetooth data
...
void setup() {
  ...
  */
  ...
  rangeNA(); // orig rangeMA(); //done in bootloader rc3105
  WDTset();
...
#elif defined BT_OUTPUT_NANOS   //line 299 Serial.print(VOUT * (RANGE_NA ? 1 : RANGE_UA ? 1000 : 1000000));
    int nanoAmps_int = (VOUT * (RANGE_NA ? 1 : RANGE_UA ? 1000 : 1000000)); Serial.print("<NA> "); Serial.print(nanoAmps_int); Serial.println(" </NA>"); //rc3105
#endif  //SerialUSB.println("\r\nReading GND and 3.3V ADC levels"); //Serial.print(VOUT * (RANGE_NA ? 1 : RANGE_UA ? 1000 : 1000000));
    nanoAmps_int = (VOUT * (RANGE_NA ? 1 : RANGE_UA ? 1000 : 1000000)); SerialUSB.print("<NA> ");  SerialUSB.print(nanoAmps_int);  SerialUSB.println(" </NA>"); //rc3105
  }  //rc3105
...
void handleTouchPads() {
  ...
  if (!AUTORANGE)
  {
    if (MA_PRESSED && UA_NOT_PRESSED && NA_NOT_PRESSED && rangeUnit != 'm') {
      rangeMA();
      rangeBeep();
    }
    if (UA_PRESSED && MA_NOT_PRESSED && NA_NOT_PRESSED && rangeUnit != 'u') {
      rangeUA();
      rangeBeep();
    }
    if (NA_PRESSED && UA_NOT_PRESSED && MA_NOT_PRESSED && rangeUnit != 'n') {
      rangeNA();
      rangeBeep();
    }
  }
  ...
  if (UA_PRESSED && NA_PRESSED && MA_NOT_PRESSED && millis() - lpfInterval > 1000) {
    toggleLPF();
    Beep(3, false);
  }

  //offset toggling (GNDISO to half supply) --- [MA+UA]
  if (MA_PRESSED && UA_PRESSED && NA_NOT_PRESSED && millis() - offsetInterval > 1000) {
    toggleOffset();
    Beep(3, false);
  }

  //AUTORANGE toggling
  if (MA_PRESSED && NA_PRESSED && UA_NOT_PRESSED && millis() - autorangeInterval > 1000) {
    toggleAutoranging();
    Beep(20, false);
    delay(50);
    (20, false);
