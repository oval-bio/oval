...
void setup() {
  ...
  pinMode(A0, 0);      pinMode(A1, 1);      pinMode(A2, 1);      //pinMode(A3, 1);      pinMode(A4, 1);
  digitalWrite(A0, 0); digitalWrite(A1, 0); digitalWrite(A2, 1); //digitalWrite(A3, 0); digitalWrite(A4, 1);
  delay(250);
  calibration_voltage = analogRead(A0) * 5.0 / 1023.0;
...
void check_airflow() {
  ...
  sensorValue = analogRead(A0); float voltage = sensorValue * 5.0 / 1023.0; float v2 = voltage - calibration_voltage;
  if ( v2 < 0.01 ) {
    v2 = 0;
  }
